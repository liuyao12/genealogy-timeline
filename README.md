...
- Hovering or keyboard-focusing a result in the left match list highlights every visible occurrence and marks the matching name text in yellow. If none is onscreen, the canvas pans smoothly only far enough to reveal the nearest occurrence and its full label; leaving the result removes the highlight without moving the canvas back. Reduced-motion preferences are respected.
- When a person appears only as a spouse in the current tree, marriages to people outside the visible tree cannot be toggled on from that household. A known marriage year is still marked by a short white line across the visible spouse's lifespan.
