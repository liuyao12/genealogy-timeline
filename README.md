...
A fresh visit immediately shows a bundled British royal line from Henry VII, pre-filtered with `king queen`. It does not contact Geni or open authorization. The historical records—including the line's confirmed spouses and Charles III's living descendant families through William and Harry—and their stable Geni profile IDs ship with the static app, so the timeline is useful even when a visitor has not authorized Geni. The left-column restore button replaces the working copy with this bundled baseline.
...
Selecting any bundled node opens its editable local copy. Its Geni panel offers **Immediate family**, **Descendants · 2 generations**, and **All descendants** imports. The first live import requires Geni authorization; after OAuth returns, the chosen operation resumes automatically and merges current public parents, spouses, children, union dates, and newly encountered profiles by Geni ID. Descendant imports render after every response so the tree visibly grows. A local relative can be attached to a Geni profile only after that profile's immediate family has been loaded. The resulting working tree, including imported records and local edits, is saved in `localStorage`.
...
- The tree-toolbar filter accepts one or more partial words or arbitrary substrings with OR semantics and searches display names and explicit titles. It keeps matching profiles and the complete ancestor/spouse paths needed to understand them, while hiding branches that contain no match.
