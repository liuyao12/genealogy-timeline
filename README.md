...
The **Timeline** settings can place a historical snapshot line at any year. Its **As of** handle stays on the sticky year ruler and can be dragged in one-year steps (or adjusted with the arrow keys). Everything beyond the line is veiled, including the future portion of a lifespan, while labels remain fully legible and change to the name, style, or title documented for that year. A living profile is labelled `(b. YEAR, age N)` at the selected year. Each profile retains a default display name plus a sourced chronological `namePeriods` list; dated names can be maintained in the profile drawer or supplied by an AI-assisted stitch import.
