...
The header can export the editable tree as JSON or save the complete rendered timeline as a high-resolution PNG. Image export includes the full off-screen canvas, year ruler, event bands, connectors, and every profile currently present under the active filter—not merely the visible viewport.
